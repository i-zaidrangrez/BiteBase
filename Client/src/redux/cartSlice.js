...
export const getCart = createAsyncThunk('cart/getcart',async (_,thunkAPI) => {
    try {
        const res = await api.get(`/cart/v1/getcart`)
        return res.data
    } catch (error) { 
        return thunkAPI.rejectWithValue(error.response?.data?.message || "Something Went Wrong") 
    }
...
export const increaseQuantity = createAsyncThunk('cart/increasequantity',async (data,thunkAPI) => {
    try {
...
export const decreaseQuantity = createAsyncThunk('cart/decreasequantity',async (data,thunkAPI) => {
    try {
...
export const clearCart = createAsyncThunk('cart/clearcart',async (data,thunkAPI) => {
    try {
